...
ipE = 500;
pertEtemp = 7*10^(-12)*TrenTriangular(TMax);
...
fgrMng.newFigure();
plot(T,pertEtemp*10^12);
grid on
hold on
plot(T,saveMatrixRaw(:,570)*10^12)
hold off
...
clim = [0,10];
imagesc([],frecuenciasRaw(1:mitad_positiva),2*abs(MatrizFFTraw))%,clim);
set(gca, 'YDir', 'normal');
xlabel("Distancia [m]")
